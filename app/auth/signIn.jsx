...
export default function SignIn() {
  ...
  const checkEmailExists = async (email) => {
    ...
    try {
      ...
      if (foundUser) {
        if (matchedRole === "users" && foundUser.disable) {
          Toast.show({
            type: "error",
            text1: "Tài khoản bị khóa",
            text2:
              "Tài khoản sinh viên này đã bị khóa. Vui lòng liên hệ Admin để được hỗ trợ.",
          });
        ...
        if (isInvalidDevice) {
          Toast.show({
            ...
            text2:
              matchedRole === "users"
                ? "Bạn đang dùng tài khoản sinh viên. \nVui lòng đăng nhập trên điện thoại."
                : "Tài khoản bạn đang dùng không dành cho sinh viên. \nVui lòng đăng nhập trên web.",
          });
          setEmailExists(false);
          setUserDetail(null);
          return;
